...
THEORY = Path(__file__).resolve().parent / "IMPORTED_TERMS.kb"
LEDGER = THEORY.parents[1]  # IMPORTED_TERMS.kb -> words-from-outside -> claims.kb
SUBPATH = LEDGER.parents[2]  # claims.kb -> dev -> docs -> kb-dynamics


def word(claim: Claim) -> str:
    """The word a claim imports, read off its slug: `load-event/` is "load event"."""
    return claim.path.parent.name.replace("-", " ")
...
def ontology_matches(defining: Claim, imports: tuple[Claim, ...]) -> list[str]:
    ...
    return [
        ...
        *(f"{THEORY}/{w.replace(' ', '-')}/: imports {w!r}, ontology omits it" for w in sorted(named - stated)),
    ]
